...
class TargetNumber3 {
    ...
    public int solution(int[] numbers, int targetNumber) {
        ...
        dfs(targetNumber, sum, 0);

        return count;
    }

    private void dfs(int targetNumber, int sum, int index) {
        if (index < numbers.length) {
            dfs(targetNumber, sum + numbers[index], index+1);
            dfs(targetNumber, sum - numbers[index], index+1);
        } else {
            if (sum == targetNumber) {
                ++count;
     ...
     */
     public static void main(String args[]) {
         TargetNumber3 tn = new TargetNumber3();
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
//// 위에 재귀로 풀었으나, 연습삼아 스택으로 다시 푸는 중 (20210808 아직 제대로 안풀어둠, 안돌아감)
//// Process
//// 1. Input numbers, targetNumber
